...
\begin{figure}[htbp]
    \begin{tikzpicture}[node distance=2.75cm, auto]
        % Define styles
        \tikzstyle{box} = [rectangle, draw, fill=pink!40, text width=8em, text centered, rounded corners, minimum height=4em]
        \tikzstyle{line} = [draw, -latex]

        % Nodes
        \node [box] (start) {Start: May 2};

        \node [box, below of=start, node distance=2.75cm] (group1_start) {Group 1 (Control): LED lights only};
        \node [box, right of=group1_start, node distance=10cm] (group2_start) {Group 2: LED + UV lights};

        \node [box, below of=group1_start, node distance=2.75cm] (seed_g1) {Plant 5 seeds of Passion \#1 (feminized) and 1 seed of Skywalker Haze (feminized)};
        \node [box, below of=group2_start, node distance=2.75cm] (seed_g2) {Plant 5 seeds of Passion \#1 (feminized) and 1 seed of Skywalker Haze (feminized)};

        \node [box, below of=seed_g1, node distance=2.75cm] (transplant_g1) {Transplant to \qty[mode=text]{15}{\L} pots on May 13};
        \node [box, below of=seed_g2, node distance=2.75cm] (transplant_g2) {Transplant to \qty[mode=text]{15}{\L} pots on May 13};

        \node [box, below of=transplant_g1, node distance=2.25cm] (frisian_g1) {Plant 5 seeds of Frisian Dew (regular) on May 13};
        \node [box, below of=transplant_g2, node distance=2.25cm] (frisian_g2) {Plant 5 seeds of Frisian Dew (regular) on May 13};

        \node [box, ultra thick, below of=frisian_g1, node distance=2.75cm] (assess) {Assess growth parameters on June 16};

        % Lines
        \path [line] (start) -- (group1_start);
        \path [line] (start) -- (group2_start);

        \path [line] (group1_start) -- (seed_g1);
        \path [line] (group2_start) -- (seed_g2);

        \path [line] (seed_g1) -- (transplant_g1);
        \path [line] (seed_g2) -- (transplant_g2);

        \path [line] (transplant_g1) -- (frisian_g1);
        \path [line] (transplant_g2) -- (frisian_g2);

        \path [line] (frisian_g1) -- (assess);
        \path [line] (frisian_g2) -- (assess);
    \end{tikzpicture}
    \caption[Sequence of experimental procedures]{Sequence of experimental procedures. Assessments include: Plant height, stem diameter, and number of internodes.}
